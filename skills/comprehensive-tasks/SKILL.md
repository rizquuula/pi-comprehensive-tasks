...
The task list is **`.pi/tasks/<session>.md`** — one per session. Two pi sessions on one project
are two pieces of work in flight, and a shared file loses one of them. `/tasks` and the tools
resolve the path themselves; read the file the widget names rather than assuming a location.

`PLAN.md` stays at the project root, shared by every session, because it is reviewed by people.
...
| Edit this session's task file | Assume the list is at the project root |
